...
export namespace SpotifyHelpers {
  ...
  export async function createUserPlaylist(
    ...
    try {
      ...
      logger.error(
        `Error creating playlist for Spotify user ${userId}: ${JSON.stringify(
          e
        )}`
      );
      return Promise.reject(e);
  ...
  export async function updateChannelPlaylistForUser(
    userId: SpotifyUser.Id,
    playlist: Playlist
  ): Promise<void> {
    const userDataStore =
      store.get<UserData.Collection>(DataStore.Keys.userData) || {};
