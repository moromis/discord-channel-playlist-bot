...
export namespace DiscordHelpers {
  ...
  export function processTracks(
    ...
    if (!_.isEmpty(trackUris)) {
      let channelPlaylistCollection =
        store.get<ChannelPlaylistCollection>(
          DataStore.Keys.channelPlaylistCollection
        ) || {};
      let channelPlaylist: Playlist =
        channelPlaylistCollection[channel.id] || Playlist.create(channel);

      // Add all Spotify URIs from the message to the playlist
      channelPlaylist.songUris.push(
        ...trackUris.map((uri) => SpotifyHelpers.encodeUri(uri))
      );
